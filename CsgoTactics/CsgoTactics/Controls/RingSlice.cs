...
namespace CsgoTactics.Controls
{
    ...
    public class RingSlice : Path
    {
        ...
        public static readonly DependencyProperty IsBackgroundRingProperty =
            DependencyProperty.Register("IsBackgroundRing", typeof(bool), typeof(RingSlice), new PropertyMetadata(true, OnIsBackgroundRingChanged));
        ...
        private void UpdatePath()
        {
            ...
            if (IsBackgroundRing)
            {
                ...
                //var outerEllipseGeometry = new EllipseGeometry();
                //var innerEllipseGeometry = new EllipseGeometry();

                //outerEllipseGeometry.Center = center;
                //outerEllipseGeometry.RadiusX = outerRadius;
                //outerEllipseGeometry.RadiusY = outerRadius;

                //innerEllipseGeometry.Center = center;
                //innerEllipseGeometry.RadiusX = innerRadius;
                //innerEllipseGeometry.RadiusX = innerRadius;

                //EllipseGeometryGroup.Children.Add(outerEllipseGeometry);
                //EllipseGeometryGroup.Children.Add(innerEllipseGeometry);

                //pathGeometry.Figures.Add(outerEllipseGeometry as PathFigure);


                var pathFigureInner = new PathFigure();
                var pathFigureOuter = new PathFigure();
                pathFigureInner.IsClosed = true;

                // Starting Point
                pathFigureInner.StartPoint = new Point(center.X - innerRadius, center.Y);
                pathFigureOuter.StartPoint = new Point(center.X - outerRadius, center.Y);

                // Inner Ellipse
                var innerArcSegment1 = new ArcSegment();
                innerArcSegment1.IsLargeArc = true;
                innerArcSegment1.Point = new Point(center.X + innerRadius, center.Y);
                innerArcSegment1.Size = new Size(innerRadius, innerRadius);
                innerArcSegment1.SweepDirection = SweepDirection.Clockwise;
                var innerArcSegment2 = new ArcSegment();
                innerArcSegment2.IsLargeArc = true;
                innerArcSegment2.Point = new Point(center.X - innerRadius, center.Y);
                innerArcSegment2.Size = new Size(innerRadius, innerRadius);
                innerArcSegment2.SweepDirection = SweepDirection.Clockwise;

                // Outer Ellipse
                var outerArcSegment1 = new ArcSegment();
                outerArcSegment1.IsLargeArc = true;
                outerArcSegment1.Point = new Point(center.X + outerRadius, center.Y);
                outerArcSegment1.Size = new Size(outerRadius, outerRadius);
                outerArcSegment1.SweepDirection = SweepDirection.Clockwise;
                var outerArcSegment2 = new ArcSegment();
                outerArcSegment2.IsLargeArc = true;
                outerArcSegment2.Point = new Point(center.X - outerRadius, center.Y);
                outerArcSegment2.Size = new Size(outerRadius, outerRadius);
                outerArcSegment2.SweepDirection = SweepDirection.Clockwise;

                pathFigureInner.Segments.Add(innerArcSegment1);
                pathFigureInner.Segments.Add(innerArcSegment2);
                pathFigureOuter.Segments.Add(outerArcSegment1);
                pathFigureOuter.Segments.Add(outerArcSegment2);
                pathGeometry.Figures.Add(pathFigureInner);
                pathGeometry.Figures.Add(pathFigureOuter);
            }
